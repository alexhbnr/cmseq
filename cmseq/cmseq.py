    ...
    def parse_gff(self, inputGFF):
        ...
        with open(inputGFF) as in_handle:
            ...
            for rec in GFF.parse(in_handle):
                if type(rec.seq) is Bio.Seq.UnknownSeq:
                    if os.path.isfile(inputGFF.replace("gff", "fna")):
                        for seq in Bio.SeqIO.parse(inputGFF.replace("gff", "fna"), "fasta"):
                            if seq.id == rec.id:
                                rec.seq = seq.seq
                                continue
                    else:
                        print('The GFF file you provided does not contain any sequences. Provide the FastA nucleotide sequence that corresponds to the GFF file at the some location with the file extension .fna')
                        sys.exit(1)
                tmp = []
                for r in rec.features:
                    ...
                    if 'gene_biotype' in r.qualifiers:
                        genbank_bool = "Genbank" in r.qualifiers['source'][0] and r.qualifiers['gene_biotype'][0] == "protein_coding"
                    else:
                        genbank_bool = False
                    
                    if prodigal_bool or genbank_bool:
